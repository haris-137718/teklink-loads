...
import {
  ...
  ChevronLeft, Copy, AlertTriangle, Eye, EyeOff, Users, Gauge, LogOut,
  Mail, ShieldCheck
} from "lucide-react";
...
.app-shell { font-family: 'Inter', system-ui, -apple-system, sans-serif; background: radial-gradient(1200px 600px at 12% -10%, rgba(255,122,51,0.24), transparent 60%), radial-gradient(1000px 520px at 100% 0%, rgba(255,171,94,0.16), transparent 55%), linear-gradient(180deg, var(--bg) 0%, var(--bg-grad-2) 100%); background-attachment: fixed; color: var(--text); min-height: 100vh; padding-bottom: 48px; }
...
.landing-badge { width: 60px; height: 60px; border-radius: 16px; background: linear-gradient(145deg, var(--accent-glow), var(--accent) 55%, var(--accent-strong)); display: flex; align-items: center; justify-content: center; color: #ffffff; box-shadow: 0 10px 32px rgba(255,122,51,0.45); }
...
.landing-card { background: linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0)), var(--panel); border: 1px solid var(--border); border-radius: 22px; padding: 44px 34px; width: 100%; max-width: 400px; box-shadow: var(--shadow); }
.landing-features { display: flex; flex-direction: column; gap: 10px; width: 100%; margin: 20px 0 6px; text-align: left; }
.landing-feature { display: flex; align-items: center; gap: 10px; font-size: 0.83rem; color: var(--text-muted); }
.landing-feature svg { color: var(--accent-glow); flex-shrink: 0; }
.landing-divider { width: 100%; height: 1px; background: var(--border); margin: 22px 0 4px; }

.auth-card { background: linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0)), var(--panel); border: 1px solid var(--border); border-radius: 22px; padding: 40px 32px; width: 100%; max-width: 400px; box-shadow: var(--shadow); text-align: left; }
.auth-icon-badge { width: 54px; height: 54px; border-radius: 15px; background: linear-gradient(145deg, var(--accent-glow), var(--accent) 55%, var(--accent-strong)); display: flex; align-items: center; justify-content: center; color: #ffffff; margin: 0 auto 18px; box-shadow: 0 8px 26px rgba(255,122,51,0.4); }
.auth-title { font-family: 'Oswald', sans-serif; text-transform: uppercase; letter-spacing: 0.03em; font-weight: 700; font-size: 1.35rem; text-align: center; margin: 0 0 6px; }
.auth-subtitle { color: var(--text-muted); font-size: 0.86rem; text-align: center; margin: 0 0 28px; }
.auth-field { margin-bottom: 16px; }
.auth-field label { display: block; font-size: 0.74rem; font-weight: 700; color: var(--text-muted); margin-bottom: 7px; text-transform: uppercase; letter-spacing: 0.05em; }
.auth-input-wrap { position: relative; display: flex; align-items: center; }
.auth-input-wrap .auth-icon-left { position: absolute; left: 14px; color: var(--text-muted); pointer-events: none; display: flex; }
.auth-input { width: 100%; background: var(--panel-alt); border: 1px solid var(--border); color: var(--text); border-radius: 11px; padding: 13px 14px 13px 42px; font-size: 0.95rem; font-family: inherit; min-height: 46px; transition: border-color 0.15s ease, box-shadow 0.15s ease; }
.auth-input:focus { outline: none; border-color: var(--accent-glow); box-shadow: 0 0 0 3px rgba(255,171,94,0.18); }
.auth-toggle-visibility { position: absolute; right: 10px; background: none; border: none; color: var(--text-muted); cursor: pointer; display: flex; align-items: center; padding: 6px; min-height: auto; }
.auth-toggle-visibility:hover { color: var(--text); }
.auth-submit { width: 100%; margin-top: 8px; }
.auth-error { background: rgba(251,113,133,0.12); border: 1px solid rgba(251,113,133,0.3); color: var(--danger); font-size: 0.83rem; border-radius: 9px; padding: 10px 12px; margin-bottom: 16px; }
...
.btn-primary { position: relative; overflow: hidden; background: linear-gradient(135deg, var(--accent-glow), var(--accent) 60%, var(--accent-strong)); color: #ffffff; box-shadow: 0 6px 18px rgba(255,122,51,0.35); }
.btn-primary::after { content: ""; position: absolute; top: 0; left: -60%; width: 40%; height: 100%; background: linear-gradient(115deg, transparent, rgba(255,255,255,0.35), transparent); transform: skewX(-20deg); transition: left 0.5s ease; }
.btn-primary:hover:not(:disabled)::after { left: 130%; }
.btn-primary:disabled { opacity: 0.5; cursor: not-allowed; box-shadow: none; }
.btn-primary:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 12px 26px rgba(255,122,51,0.5); }
...
.stat-card { background: var(--panel); border: 1px solid var(--border); border-radius: 12px; padding: 14px; box-shadow: var(--shadow-sm); transition: transform 0.2s ease, box-shadow 0.2s ease, border-color 0.2s ease; animation: cardIn 0.4s cubic-bezier(.2,.8,.2,1) backwards; }
.stat-card:hover { transform: translateY(-2px); box-shadow: var(--shadow); border-color: var(--accent-glow); }
.stat-grid .stat-card:nth-child(1){ animation-delay: 0.03s; } .stat-grid .stat-card:nth-child(2){ animation-delay: 0.08s; } .stat-grid .stat-card:nth-child(3){ animation-delay: 0.13s; }
...
.gauge-row { display: flex; flex-direction: column; align-items: center; gap: 18px; background: var(--panel); border: 1px solid var(--border); border-radius: 14px; padding: 18px; margin-bottom: 16px; box-shadow: var(--shadow-sm); animation: cardIn 0.45s cubic-bezier(.2,.8,.2,1) 0.16s backwards; }
.tab-fade-in { animation: cardIn 0.3s cubic-bezier(.2,.8,.2,1); }
...
.load-card { background: var(--panel); border: 1px solid var(--border); border-radius: 12px; padding: 14px; box-shadow: var(--shadow-sm); transition: box-shadow 0.2s ease, transform 0.2s ease, border-color 0.2s ease; animation: cardIn 0.35s cubic-bezier(.2,.8,.2,1) backwards; }
.load-card:hover { box-shadow: var(--shadow); transform: translateY(-2px); border-color: var(--accent-glow); }
...
.badge-btn { background: none; border: none; padding: 0; min-height: unset; transition: transform 0.15s cubic-bezier(.34,1.56,.64,1); }
.badge-btn:active { transform: scale(0.9); }
.badge { display: inline-flex; align-items: center; gap: 4px; padding: 5px 10px; border-radius: 999px; font-size: 0.72rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.02em; transition: background-color 0.25s ease, color 0.25s ease; }
.badge-paid { background: rgba(62,207,142,0.16); color: var(--success); }
.badge-unpaid { background: rgba(255,122,51,0.16); color: var(--accent-glow); }
...
.modal-overlay { position: fixed; inset: 0; background: rgba(3,6,14,0.72); display: flex; align-items: center; justify-content: center; padding: 16px; z-index: 50; animation: overlayIn 0.2s ease; }
@keyframes overlayIn { from { opacity: 0; } to { opacity: 1; } }
.modal-panel { background: var(--panel); border: 1px solid var(--border); border-radius: 16px; width: 100%; max-width: 480px; max-height: 90vh; overflow-y: auto; box-shadow: 0 24px 60px rgba(0,0,0,0.55); animation: panelIn 0.25s cubic-bezier(.2,.8,.2,1); }
...
.tab-btn { background: none; border: none; color: var(--text-muted); padding: 10px 14px; font-weight: 600; font-size: 0.85rem; border-bottom: 2px solid transparent; min-height: 40px; transition: color 0.2s ease, border-color 0.2s ease; }
...
.toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: var(--panel-alt); border: 1px solid var(--border); color: var(--text); padding: 12px 18px; border-radius: 10px; font-size: 0.85rem; z-index: 100; box-shadow: 0 8px 24px rgba(0,0,0,0.4); animation: toastIn 0.3s cubic-bezier(.2,.8,.2,1); }
@keyframes toastIn { from { opacity: 0; transform: translateX(-50%) translateY(14px); } to { opacity: 1; transform: translateX(-50%) translateY(0); } }
.toast-error { border-color: var(--danger); color: var(--danger); }
.toast-success { border-color: var(--success); }

.spin { animation: spin 1s linear infinite; }
.load-screen-badge{width:58px; height:58px; border-radius:16px; background:linear-gradient(145deg,var(--accent-glow),var(--accent) 55%,var(--accent-strong)); display:flex; align-items:center; justify-content:center; color:#fff; box-shadow:0 10px 30px rgba(255,122,51,0.4); margin-bottom:6px; animation:loadPulse 1.6s ease-in-out infinite;}
@keyframes loadPulse{ 0%,100%{ transform:scale(1); } 50%{ transform:scale(1.08); } }
.load-screen-track{position:relative; width:180px; height:3px; background:var(--border); border-radius:3px; overflow:hidden; margin-top:4px;}
.load-screen-track::after{content:""; position:absolute; top:0; left:0; height:100%; width:40%; background:linear-gradient(90deg, transparent, var(--accent-glow), var(--accent), transparent); animation:loadTrack 1.3s ease-in-out infinite;}
@keyframes loadTrack{ 0%{ left:-40%; } 100%{ left:100%; } }
.load-screen-text{font-family:'Oswald', sans-serif; text-transform:uppercase; letter-spacing:0.08em; font-size:0.85rem; color:var(--text-muted); margin-top:2px;}
...
function LoadCard({ load, carrierName, isDispatcher, onEdit, onDelete, onDuplicate, onToggleFee, delay = 0 }) {
  ...
  return (
    <div className="load-card" style={{ animationDelay: `${Math.min(delay, 12) * 0.04}s` }}>
...
export default function App() {
  ...
  const [showCarrierPassword, setShowCarrierPassword] = useState(false);
  ...
  if (loading) {
    return (
      <div className="app-shell center-screen">
        <style>{CSS}</style>
        <div className="load-screen-badge"><Truck size={26} /></div>
        <div className="load-screen-text">Loading</div>
        <div className="load-screen-track"></div>
      </div>
    );
  }
        ...
        <div className="landing center-screen">
          <div className="landing-card">
            ...
            <div className="landing-features">
              <div className="landing-feature"><CheckCircle2 size={15} /> Real-time load &amp; revenue tracking</div>
              <div className="landing-feature"><CheckCircle2 size={15} /> Secure carrier &amp; dispatcher access</div>
              <div className="landing-feature"><CheckCircle2 size={15} /> Centralized invoice management</div>
            </div>
            <div className="landing-divider" />
            <div className="landing-actions" style={{ maxWidth: "none" }}>
      ...
      {screen === "carrierLogin" && (
        <div className="screen-pad center-screen">
          ...
          <div className="auth-card">
            <div className="auth-icon-badge"><ShieldCheck size={24} /></div>
            <h2 className="auth-title">Carrier Login</h2>
            <p className="auth-subtitle">Sign in to view your loads, fees, and invoices</p>
            {carrierLoginError && <div className="auth-error">{carrierLoginError}</div>}
            <div className="auth-field">
              <label>Email address</label>
              <div className="auth-input-wrap">
                <span className="auth-icon-left"><Mail size={17} /></span>
                <input
                  type="email" className="auth-input" value={carrierEmailInput}
            ...
            <div className="auth-field">
              ...
              <div className="auth-input-wrap">
                <span className="auth-icon-left"><Lock size={17} /></span>
                <input
                  type={showCarrierPassword ? "text" : "password"} className="auth-input" style={{ paddingRight: 40 }} value={carrierPasswordInput}
                  onChange={(e) => { setCarrierPasswordInput(e.target.value); setCarrierLoginError(""); }}
                  onKeyDown={(e) => e.key === "Enter" && handleCarrierLoginSubmit()}
                  placeholder="Enter your password"
                />
                <button type="button" className="auth-toggle-visibility" onClick={() => setShowCarrierPassword((s) => !s)} tabIndex={-1}>
                  {showCarrierPassword ? <EyeOff size={17} /> : <Eye size={17} />}
                </button>
              </div>
            </div>
            <button className="btn-primary btn-lg auth-submit" onClick={handleCarrierLoginSubmit}>Log in</button>
          </div>
        </div>
      )}
        ...
        return (
          <div className="screen-pad">
            ...
            <div className="load-list">
              ...
              ) : carrierLoads.map((l, i) => (
                <LoadCard key={l.id} load={l} carrierName={carrier ? carrier.name : ""} isDispatcher={false} delay={i} />
              ))}
            </div>
          </div>
        );
      ...
      {screen === "dispatcherPin" && (
        <div className="screen-pad center-screen">
          ...
          <div className="auth-card" style={{ textAlign: "center" }}>
            <div className="auth-icon-badge"><Lock size={22} /></div>
            <h2 className="auth-title">Dispatcher Login</h2>
            <p className="auth-subtitle">Enter your PIN to access the dashboard</p>
            <input
            ...
            />
            {pinError && <div className="error-text" style={{ marginTop: 10 }}>{pinError}</div>}
            <button className="btn-primary btn-lg auth-submit" style={{ marginTop: 18 }} onClick={handlePinSubmit}>Unlock</button>
          </div>
        </div>
      )}
        ...
        return (
          <div className="screen-pad">
            ...
            {dispatcherTab === "invoices" && (
              <div className="load-list tab-fade-in">
                {carriers.length === 0 ? (
            ...
            {dispatcherTab === "carriers" && (
              <div className="carriers-panel tab-fade-in">
